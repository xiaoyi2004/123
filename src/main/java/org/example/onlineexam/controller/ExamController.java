...
public class ExamController {
    ...
    public String takeExam(@PathVariable Long examId, HttpSession session, Model model) {
        ...
        if (exam == null)
            return "redirect:/student/dashboard?error=examNotFound";
    ...
    public String submitExam(@PathVariable Long examId, HttpSession session, HttpServletRequest request) throws JsonProcessingException {
        ...
        for (PaperQuestion pq : pqs) {
            ...
            if (q != null) { questions.add(q); scoreMap.put(q.getId(), pq.getScore()==null?0:pq.getScore());
                answers.put(String.valueOf(q.getId()), Optional.ofNullable(request.getParameter("q_" + q.getId())).orElse(""));
            }
        }
        int objective = gradingService.gradeObjective(questions, scoreMap, answers);
        boolean hasSubjective = gradingService.hasSubjective(questions);
        ExamResult result = new ExamResult(); result.setStudentId(user.getId()); result.setExamId(examId);
        result.setObjectiveScore(objective); result.setSubjectiveScore(0); result.setScore(objective);
        result.setGradeStatus(hasSubjective ? "待批阅" : "已批阅"); result.setAnswersJson(objectMapper.writeValueAsString(answers));
        result.setSubmitTime(LocalDateTime.now()); examResultRepository.save(result);
